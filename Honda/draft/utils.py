...
def hex_to_binary(hex_str, pad_to=1):
    """ 
...
def mask_hex_str(hex_str, position, length, pad_to=64, byteorder='big', signed=False):
    ...
    # previously, would only calculate for endianess and signing if the original length was a multiple of 8 bits
#     if length % 8 == 0:
#         # need to convert to byte representation, read using proper endianness and signing, and then go back to decimal
#         length_in_bytes = length // 8
#         byte_representation = int(binary_representation, 2).to_bytes(length_in_bytes, byteorder='big')
#         int_representation = int.from_bytes(byte_representation, byteorder=byteorder, signed=signed)
#     else:
#         # directly convert hex to binary, mask, then convert to int
#         int_representation = int(hex_to_binary(hex_str, pad_to=pad_to)[position:position+length], 2)
...
def get_raw_signal_values(csv_data, message_id, position, length):
    ...
    for bus in buses:
        ...
        y = message_data_for_bus['Message'].apply(
            lambda hex_str: hex_to_binary(hex_str, pad_to=pad_to)[position:position+length])
        xs.append(x)
        ys.append(y)
    return xs, ys


def plot_message_id(csv_data, message_id, position, length, byteorder='big', signed=False, scale=1, offset=0, plot_fxn=plt.scatter):
    message_data = csv_data.where(csv_data['MessageID'] == message_id)
    buses = message_data['Bus'].unique()
    xs, ys = [], []
...
def describe_known_signal(cantools_db, message_name, signal_name, csv_data=None, plot_fxn=plt.scatter, silent=False):
    ...
    if not silent:
        print(f"message name: {message.name}\n"
              ...
              f"message layout_string: \n{message.layout_string()}\n"
             )
    ...
    if csv_data is not None:
        xs, ys = plot_message_id(csv_data, message.frame_id, start, signal.length, byte_order, signal.is_signed, signal.scale, signal.offset, plot_fxn)
    else:
        xs, ys = None, None
